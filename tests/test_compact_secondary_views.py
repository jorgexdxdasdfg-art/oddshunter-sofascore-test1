...
def test_deployment_requires_the_compact_secondary_marker():
    ...
    assert "1.26.6-real-odds-precision" in recovery
...
def test_picks_read_persisted_current_odds_and_all_picks_schema():
    ...
    assert "pick?.current_odds??pick?.source_odds??pick?.odds" in script
    assert "app.js?v=1.26.6-real-odds-precision" in recovery
    assert "app.css?v=1.26.6-real-odds-precision" in recovery
    assert "oh-mobile-v1-26-6-real-odds-precision" in recovery
    assert 'sw.js?v=1.26.6-real-odds-precision' in recovery
    assert 'name="oddshunter-build" content="1.26.6-real-odds-precision"' in recovery
    assert 'dataset.oddshunterBuild="1.26.6-real-odds-precision"' in script
