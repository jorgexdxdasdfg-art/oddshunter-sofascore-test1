...
def test_deployment_requires_the_compact_secondary_marker():
    ...
    assert "1.26.5-under-source-label" in recovery
...
def test_picks_read_persisted_current_odds_and_all_picks_schema():
    ...
    assert "app.js?v=1.26.5-under-source-label" in recovery
    assert "app.css?v=1.26.5-under-source-label" in recovery
    assert "oh-mobile-v1-26-5-under-source-label" in recovery
    assert 'sw.js?v=1.26.5-under-source-label' in recovery
    assert 'name="oddshunter-build" content="1.26.5-under-source-label"' in recovery
    assert 'dataset.oddshunterBuild="1.26.5-under-source-label"' in script
