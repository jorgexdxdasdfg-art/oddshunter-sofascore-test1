...
def patch_frontend(root: Path) -> dict[str, list[str]]:
    ...
    for path in app_candidates:
        ...
        text = text.rstrip() + "\n\n" + detail_js.rstrip() + "\n"
        text = re.sub(r"sw\.js\?v=[^\"']+", "sw.js?v=1.29.0-complete-fixtures-results", text)
        if text != original:
    ...
    for path in sorted(root.glob("**/index.html")):
        text = path.read_text(encoding="utf-8")
        updated = re.sub(r"app\.css\?v=[^\"']+", "app.css?v=1.29.0-complete-fixtures-results", text)
        updated = re.sub(r"app\.js\?v=[^\"']+", "app.js?v=1.29.0-complete-fixtures-results", updated)
        updated = re.sub(r"sw\.js\?v=[^\"']+", "sw.js?v=1.29.0-complete-fixtures-results", updated)
        updated = re.sub(r'\s*<meta\s+name=["\']oddshunter-build["\'][^>]*>', "", updated)
        updated = updated.replace("</head>", '  <meta name="oddshunter-build" content="1.29.0-complete-fixtures-results">\n</head>', 1)
    ...
    for path in sorted(root.glob("**/sw.js")):
        text = path.read_text(encoding="utf-8")
        updated = re.sub(r"oh-mobile-v[^\"']+", "oh-mobile-v1-29-0-complete-fixtures-results", text)
        updated = re.sub(r"app\.css\?v=[^\"']+", "app.css?v=1.29.0-complete-fixtures-results", updated)
        updated = re.sub(r"app\.js\?v=[^\"']+", "app.js?v=1.29.0-complete-fixtures-results", updated)
        if updated != text:
            path.write_text(updated, encoding="utf-8", newline="\n")
