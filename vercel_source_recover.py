...
DETAIL_MARKER = "OH_MATCH_REFINEMENTS_V6"
...
def patch_frontend(root: Path) -> dict[str, list[str]]:
    ...
    for path in sorted(root.glob("**/index.html")):
        text = path.read_text(encoding="utf-8")
        updated = re.sub(r"app\.js\?v=[^\"']+", "app.js?v=1.6.4-summary-refinements", text)
        updated = re.sub(r"sw\.js\?v=[^\"']+", "sw.js?v=1.6.4-summary-refinements", updated)
        if updated != text:
            path.write_text(updated, encoding="utf-8", newline="\n")
    ...
    for path in sorted(root.glob("**/sw.js")):
        text = path.read_text(encoding="utf-8")
        updated = re.sub(r"oh-mobile-v[^\"']+", "oh-mobile-v1-6-4-summary-refinements", text)
        if updated != text:
            path.write_text(updated, encoding="utf-8", newline="\n")
