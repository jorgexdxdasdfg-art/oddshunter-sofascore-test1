...
def patch_frontend(root: Path) -> dict[str, list[str]]:
    ...
    for path in app_candidates:
        ...
        text = text.rstrip() + "\n\n" + detail_js.rstrip() + "\n"
        text = re.sub(r"sw\.js\?v=[^\"']+", "sw.js?v=1.30.0-complete-ev-top60", text)
        if text != original:
    ...
    for path in sorted(root.glob("**/index.html")):
        text = path.read_text(encoding="utf-8")
        updated = re.sub(r"app\.css\?v=[^\"']+", "app.css?v=1.30.0-complete-ev-top60", text)
        updated = re.sub(r"app\.js\?v=[^\"']+", "app.js?v=1.30.0-complete-ev-top60", updated)
        updated = re.sub(r"sw\.js\?v=[^\"']+", "sw.js?v=1.30.0-complete-ev-top60", updated)
        updated = re.sub(r'\s*<meta\s+name=["\']oddshunter-build["\'][^>]*>', "", updated)
        updated = updated.replace("</head>", '  <meta name="oddshunter-build" content="1.30.0-complete-ev-top60">\n</head>', 1)
    ...
    for path in sorted(root.glob("**/sw.js")):
        text = path.read_text(encoding="utf-8")
        updated = re.sub(r"oh-mobile-v[^\"']+", "oh-mobile-v1-30-0-complete-ev-top60", text)
        updated = re.sub(r"app\.css\?v=[^\"']+", "app.css?v=1.30.0-complete-ev-top60", updated)
        updated = re.sub(r"app\.js\?v=[^\"']+", "app.js?v=1.30.0-complete-ev-top60", updated)
        if updated != text:
            path.write_text(updated, encoding="utf-8", newline="\n")
